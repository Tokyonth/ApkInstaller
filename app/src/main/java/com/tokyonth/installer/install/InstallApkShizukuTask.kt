...
class InstallApkShizukuTask : MakeInstaller() {
    ...
    private fun getPackageInstaller(): PackageInstaller {
        ...
        val installerPackageName = if (isRoot) App.context.packageName else "com.android.shell"
    ...
    private fun getPackageInstallerSession(totalSize: Long? = null): PackageInstaller.Session {
        ...
        val _session = IPackageInstallerSession.Stub.asInterface(
            ShizukuBinderWrapper(
                ShizukuSystemServerApi.PackageManager_getPackageInstaller().openSession(sessionId)
                    .asBinder()
            )
        )
